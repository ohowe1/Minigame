...
public abstract class CommandManager {
    ...
    protected List<String> processTabComplete(CommandSender sender, String[] args) {
        List<String> possibleCommandNames = commands.stream().filter(
                command -> shouldTabComplete(command, sender)
        ).map(GlobalCommand::getName).collect(Collectors.toList());
        ...
        if (args.length == 1) {
            ...
            for (GlobalCommand globalCommand : commands) {
                ...
                if (!sender.hasPermission(globalCommand.getPermission()) && !globalCommand.getName().isEmpty()) {
                    continue;
                }
    ...
    protected void processCommand(CommandSender sender, String[] args) {
        ...
        for (GlobalCommand globalCommand : commands) {
            ...
            if (globalCommand.getPermission() != null) {
                if (!sender.hasPermission(globalCommand.getPermission()) && !globalCommand.getName().isEmpty() && !(globalCommand.getName() == null)) {
