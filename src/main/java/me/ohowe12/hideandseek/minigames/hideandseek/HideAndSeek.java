...
import me.ohowe12.hideandseek.minigames.MiniGame;
...
public class HideAndSeek extends MiniGame implements PlayerCommand {
    private final PotionEffect INVIS = new PotionEffect(PotionEffectType.INVISIBILITY, 1000000, 255, false, false, true);
    private Player seeker;
    private Team seekers;
    private int seconds;
    private boolean canChangeSeeker = true;
    ...
    private Location getClosestPlayer() {
        ...
        for (Player p : hiders) {
            if (p.getLocation().distanceSquared(seeker.getLocation()) < closet.distanceSquared(seeker.getLocation())) {
    ...
    public HideAndSeek(ArrayList<Player> players, Plugin plugin, World world, MiniGameManager manager) {
    ...
    public void start() {
        ...
        hiders.setOption(org.bukkit.scoreboard.Team.Option.NAME_TAG_VISIBILITY, org.bukkit.scoreboard.Team.OptionStatus.FOR_OTHER_TEAMS);
        ...
        border.setSize(1000);
        hiders.forEach(
                player -> player.addPotionEffect(INVIS)
        );
    ...
    public void onMove(PlayerMoveEvent e) {
        e.setCancelled(seconds < 45 && e.getPlayer().equals(seeker) && running);
    }

    private void startGame() {
        canChangeSeeker = false;
    ...
    protected void second() {
        ...
        if (seconds % 300 == 0) {
            sendToAllPlayers(ChatColor.RED + "Shrinking world border in half!", "The center is " + center.getX() + " " + center.getZ());
