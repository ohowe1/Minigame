...
public class Language {
    ...
    public String getColorizedHeader(String name) {
        return ChatColor.translateAlternateColorCodes('&', getTranslation("headers", name));
    }
    ...
    private JsonObject getJsonObject(@Nullable String language) {
        ...
        if (!languages.contains(language)) {
            plugin.getPluginLogger().log(Logger.ANSI_RED + "That is an invalid language! Using en-US.");
            language = "en-US";
        }
    ...
    private JsonObject getCategory(String category, JsonObject languageJsonToUse) {
        ...
        if (jsonCategory == null) {
            jsonCategory = getJsonObject("en-US").get(category);
            if (jsonCategory == null) {
                badLanguage();
                return null;
            }
    ...
    private void badLanguage() {
        ...
        plugin.getPluginLogger().log(Logger.ANSI_RED + "If you did not mess with the .jar at all please report it to the developer!");
    }
}
